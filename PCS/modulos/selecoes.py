...
class Selecoes(): #to começando a achar que essa classe ta maior doq deveria mas fazer oq
    ...
    def checaClique(self, tecla):
        if tecla == K_z and alma.rect.colliderect(self.botao):
            clicaAcao = pygame.mixer.Sound('UnderGaragem-Baixar-e-jogar\\PCS\\assets\\sounds\\snd_select.mp3')
    ...
    def mirar(self, tecla):
        ...
        if self.mirando:
            #print('mirar inicia')
            self.alvo = pygame.image.load('UnderGaragem-Baixar-e-jogar\\PCS\\assets\\sprites\\mira.png')
    ...
    def animaAtaque(self):
        ...
        if self.animandoAtaque:
            ...
            if self.framesAtaque >= 0 and cos.tocouAtaque == False:
                ataqueSom = pygame.mixer.Sound('UnderGaragem-Baixar-e-jogar\\PCS\\assets\\sounds\\slash.mp3')
                ataqueSom.play()
